...
try {
    ...
    if (!isset($_POST['event_id']) || !isset($_POST['title']) || !isset($_POST['type']) || !isset($_POST['date']) || !isset($_POST['start_time']) || empty($_POST['start_time']) || !isset($_POST['end_time']) || empty($_POST['end_time'])) {
    ...
    // Check if event exists and was created by current user, and fetch its date/time
    $check_query = "SELECT created_by, event_date, event_time FROM events WHERE event_id = $event_id";
    ...
    if ($event_data['created_by'] != $user_id) {
        throw new Exception('You can only edit events you created');
    }

    // Prevent editing of expired events
    ...
    // Ensure event_time column exists (safety for legacy databases)
    ...
    // Update the event (using start_time as event_time for backward compatibility)
    $query = "UPDATE events SET 
              ...
              WHERE event_id = $event_id AND created_by = $user_id";
